...
void Camera::processmousescroll(GLfloat delta) { _position += delta * _front; }

float &Camera::zoom() { return _zoom; }

glm::vec3 &Camera::position() { return _position; }

void Camera::setviewport(glm::vec4 viewport) { _viewport = viewport; }

/*------------------------------------------------------------------------------------------------------------------------*/

// A camera class that processes input and calculates the corresponding Eular
// Angles, Vectors and Matrices for use in OpenGL Constructor with vectors
...
void EulerCamera::processmousemovement(int button, GLfloat xpos, GLfloat ypos,
...
/*------------------------------------------------------------------------------------------------------------------------*/
TrackballCamera::TrackballCamera(glm::vec3 position, glm::vec3 up, GLfloat yaw,
                                 GLfloat pitch)
    : Camera(position, up, position + glm::vec3(0.0f, 0.0f, -1.0f), ZOOM),
      _movementspeed(SPEED), _mousesensitivity(SENSITIVTY) {
  _worldup = _up;
...
// parameter in the form of camera defined ENUM (to abstract it from windowing
// systems)
void TrackballCamera::processkeyboard(Camera_Movement direction,
                                      GLfloat deltaTime) {
  GLfloat velocity = _movementspeed * deltaTime;
  if (direction == FORWARD)
    _position += _front * velocity;
...
void TrackballCamera::processmouseclick(int button, GLfloat xpos,
                                        GLfloat ypos) {
...
// Processes input received from a mouse input system. Expects the offset value
// in both the x and y direction.
void TrackballCamera::processmousemovement(int button, GLfloat xpos,
                                           GLfloat ypos, GLboolean) {
  float xoffset = xpos - _mousestartx;
  float yoffset = _mousestarty - ypos;
  _mousestartx = xpos;
  _mousestarty = ypos;
