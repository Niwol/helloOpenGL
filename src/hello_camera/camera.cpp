...
// // A camera class that processes input and calculates the corresponding Eular
// Angles, Vectors and Matrices for use in OpenGL
// // Constructor with vectors
// EulerCamera::EulerCamera(glm::vec3 position, glm::vec3 up, GLfloat yaw,
// GLfloat pitch) :
//             Camera(position, up, position+glm::vec3(0.0f, 0.0f, -1.0f),
//             ZOOM), _movementspeed(SPEED), _mousesensitivity(SENSITIVTY) {
//     _worldup = _up;
//     _yaw = yaw;
//     _pitch = pitch;
//     updatecameravectors();
...
// windowing systems) void EulerCamera::processkeyboard(Camera_Movement
...
// value in both the x and y direction. void
// EulerCamera::processmousemovement(int button, GLfloat xpos, GLfloat ypos,
...
//     _right = glm::normalize(glm::cross(_front, _worldup));  // Normalize the
//     vectors, because their length gets closer to 0 the more you look up or
//     down which results in slower movement. _up    =
//     glm::normalize(glm::cross(_right, _front));
...
// /*------------------------------------------------------------------------------------------------------------------------*/
// TrackballCamera::TrackballCamera(glm::vec3 position, glm::vec3 up, GLfloat
// yaw, GLfloat pitch) :
...
// // Processes input received from any keyboard-like input system. Accepts
// input parameter in the form of camera defined ENUM (to abstract it from
// windowing systems) void TrackballCamera::processkeyboard(Camera_Movement
// direction, GLfloat deltaTime) {
//     GLfloat velocity = _movementspeed * deltaTime;
//     if (direction == FORWARD)
//         _position += _front * velocity;
...
// void TrackballCamera::processmouseclick(int button, GLfloat xpos, GLfloat
// ypos) {
...
// // Processes input received from a mouse input system. Expects the offset
// value in both the x and y direction. void
// TrackballCamera::processmousemovement(int button, GLfloat xpos, GLfloat ypos,
// GLboolean ) {
//     float xoffset = xpos - _mousestartx;
//     float yoffset = _mousestarty - ypos;
//     _mousestartx = xpos;
...
//     vectors, because their length gets closer to 0 the more you look up or
//     down which results in slower movement. _up    =
//     glm::normalize(glm::cross(_right, _front)); _worldup = _up;
