...
MyOpenGLWidget::MyOpenGLWidget(QWidget *parent)
  ...
  _democonstructors.push_back([](int width, int height) -> OpenGLDemo * {
    ...
    return new BezierDemo(width, height);
  });
}

MyOpenGLWidget::~MyOpenGLWidget() {}

QSize MyOpenGLWidget::minimumSizeHint() const { return QSize(50, 50); }

QSize MyOpenGLWidget::sizeHint() const { return QSize(512, 512); }

void MyOpenGLWidget::cleanup() { _openglDemo.reset(nullptr); }
...
void MyOpenGLWidget::initializeGL() {
  ...
  if (!initializeOpenGLFunctions()) {
    QMessageBox::critical(this, "OpenGL initialization error",
                          "MyOpenGLWidget::initializeGL() : Unable to "
